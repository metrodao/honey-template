...
const createDao = async (honeyPotTemplate) => {
  ...
  const createDaoTxOneReceipt = await honeyPotTemplate.createDaoTxOne(
    ...
    BRIGHTID_VERIFICATIONS_REQUIRED
  );
  ...
  } else if (oldConfig.hookedTokenManagerAddress !== "") {
    const oldHookedTokenManager = await HookedTokenManager.at(oldConfig.hookedTokenManagerAddress)
